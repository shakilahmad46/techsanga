...
?>
<div id="stregtui_slider_single" style="background-size: cover; margin:0 auto;width:<?php print $width ?>; height: <?php print $height ?>; border: 1px solid #ccc; list-style: none;position: relative;">

</div>

<div class="vertical-tabs clearfix">
  <div class="vertical-tabs-list">
  <ul id="stregtui_list_layers">
  </ul>
  ...
  <fieldset class="form-wrapper g-wrapper">
      <div class="stregtui-heading">Layer Transition Effects</div>
        ...
        <tr>
          <td width="25%"><label>Incoming Effect</label>
            <select name="incomingclasses" class="layer-option">
              <?php foreach (stregtui GetArrAnimations() as $key => $value) { ?>
          ...
          <td>
            <label>Easing</label>
            <?php print stregtui_defined_select('data_easing', stregtui_sliderlayer_dataeasing(),'layer-option'); ?></td>
          </td>
          <td  width="25%">
        ...
        <tr>
          <td><label>Outgoing Effect</label>
            <select name="outgoingclasses" class="layer-option">
                <?php foreach (stregtui GetArrEndAnimations() as $key => $value) { ?>
                    <option value="<?php print $key ?>" <?php if(isset($value['disable']) && ($value['disable']==true)) print 'disabled' ?>><?php print $value['handle']; ?></option>
               <?php } ?>
            </select>
          </td>
          <td>
            ...
            <?php print stregtui_defined_select('data_endeasing', stregtui_sliderlayer_dataendeasing(),'layer-option'); ?>
          </td>
          <td width="25%">
  ...
  <fieldset class="fieldset-wrapper g-wrapper">
      <div class="stregtui-heading">Layer Style Setting</div>
            ...
            <td colspan="3">
              <div id="content-type">
                    ...
                    <table>
                      <tr>
                        <td width="50%">
                          ...
                          <?php print stregtui_defined_select('text_style', stregtui_sliderlayer_captionclasses(),'layer-option'); ?>
                        </td>
                        <td width="50%">
                ...
                <div id="content-image" class="g-content-setting filed-upload-layer-content" style="display: none;">
                  <?php print stregtui_sliderlayer_field_upload_layer(); ?>
    ...
    <fieldset class="fieldset-wrapper g-wrapper">
      <div class="stregtui-heading">Layer Setting</div>
      <table>
        <tr>
          <td width="33%">
          ...
          <?php if(stregtui_sliderlayer_styles()){
            $styles = stregtui_sliderlayer_styles();
          ?>
            <td>
