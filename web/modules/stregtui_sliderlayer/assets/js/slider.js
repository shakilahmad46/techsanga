var $settings = drupalSettings.stregtui_sliderlayer.settings;

var $group_settings = drupalSettings.stregtui_sliderlayer.group_settings;

var $layers = drupalSettings.stregtui_sliderlayer.layers_settings;
...
var delayer = drupalSettings.stregtui_sliderlayer.delayer;

var deslider = drupalSettings.stregtui_sliderlayer.deslider;
...
(function ($) {
  ...
  var slider = document.getElementById('g-slider');
   ...
   $('#g_data_end').on('change', function() {
    ...
    $('#stregtui_slider_single').width($group_settings.startwidth).height($group_settings.startheight);
    ...
    $('input#g-image-layer').on('onchange', function(){
        $url = drupalSettings.stregtui_sliderlayer.base_url + $(this).val();
  ...
  function setSlideSackground(url){
    jQuery('#stregtui_slider_single').css({
...
//========================================
  ...
  function load_slider() {
    ...
    if ($settings.background_image_uri != '') {
      $('#stregtui_slider_single').css({
        'background-image': 'url(' + drupalSettings.stregtui_sliderlayer.base_url  + $settings.background_image_uri + ')'
      })
      $('.field-upload-background-slider .stregtui-image-demo').attr('src', drupalSettings.stregtui_sliderlayer.base_url  + $settings.background_image_uri);
  ...
  function load_layers() {
    $('#stregtui_slider_single').find('div').remove();
    $('#stregtui_list_layers').find('li').remove();
  ...
  function gva_sl_add_tab_layer(i) {
     ...
     $('ul#stregtui_list_layers').append(t);
     ...
     switch ($layers[i].type) {
        case 'image':
        var url = drupalSettings.stregtui_sliderlayer.base_url  + $layers[i].image_uri;
          content = '<img src="' + url + '" />';
          var img = new Image();
          img.onload = function() {
          ...
          img.src = drupalSettings.stregtui_sliderlayer.base_url  + $layers[i].image_uri;
          break;
        case 'text':
     ...
     $('#stregtui_slider_single').append(newdelayer);
     ...
     $('#stregtui_list_layers').sortable({
           handle: '.move',
           update: function (event, ui) {
              $('#stregtui_list_layers').find('li').each(function (index) {
                    var lindex = $(this).attr('index');
                    $layers[lindex].index = index;
                    $('#layer-'+lindex).css({zIndex:(99-index)});
                    save_layer();
              });
              $layers.sort(stregtuiCompare);
              save_slider();
              load_slider();
           }
     });
 ...
 function load_type_layer(type){
    ...
    $('ul#stregtui_list_layers li.active').removeClass('image text').addClass(type);
    if (type == 'image') {
  ...
  function load_layer(i) {
    ...
    $('ul#stregtui_list_layers').find('li').removeClass('active');
    $('ul#stregtui_list_layers').find('li[index=' + i + ']').addClass('active');
  ...
  function remove_layer(i) {
    $('#layer-' + i).remove();
    $layers[i]['removed'] = 1;
    $('ul#stregtui_list_layers').find('li[index=' + i + ']').remove();
    if (i == $cxt) {
      if($('ul#stregtui_list_layers li').length > 0){
        var f = parseInt($('ul#stregtui_list_layers').find('li:first').attr('index'));
        load_layer(f);
      }
    }
  }
  ...
  function save_layer_slider() {
    ...
    $.ajax({
      url: drupalSettings.stregtui_sliderlayer.save_url,
      type: 'POST',
      data: data,
      dataType: 'json',
   ...
   function load_imce(){
    ...
    //Select IMCE file, display image demo and set val for upload input.
      ...
      $('.imce-url-input').not('.imce-url-processed').addClass('imce-url-processed').each(function(){
        ...
        el.before('<a href="#" class="imce-url-button"><span>Open File Browser</span></a> | <a href="#" class="stregtui-field-upload-remove">Remove</a>');
      });
      ...
      var gvaImceInputSlider = window.gvaImceInputSlider = window.gvaImceInputSlider || {
         urlSendto: function(File, win) {
          ...
          win.close();
          if (el) {
            var base_path = drupalSettings.stregtui_sliderlayer.base_path;
            var url_new = '/' + url.replace(base_path, '');
            console.log(url);

            $(el).val(url_new);
            $(el).parents('.gva-upload-input').find('.stregtui-image-demo').attr('src', url);
            $('#stregtui_slider_single').css('background-image', 'url(\'' + url + '\')');
          }
        }
      };

      $(document).ready(function(){
        $('.stregtui-field-upload-remove').on('click', function(e){
          e.preventDefault();
          $(this).parents('.gva-upload-input').find('.stregtui-image-demo').attr('src', '');
          $(this).parents('.gva-upload-input').find('.imce-url-input').val('');
          $('#stregtui_slider_single').css({'background-image': 'url("")'});
        });
      })
