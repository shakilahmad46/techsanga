<?php
function stregtui_sliderlayer_block_content($sid) {
  global $base_url;
  $slideshow = stregtui_slider_load_frontend($sid);
  if(!$slideshow) return 'No slider selected';
  $module_path = drupal_get_path('module', 'stregtui_sliderlayer');
  ...
  return stregtui_sliderlayer_slides($slide_settings);

}

function stregtui_sliderlayer_slides($vars){
  global $base_url;
  $slides = $vars['slides'];
  $scount = $vars['scount'];
  $settings = $vars['settings'];
  $vars['attributes_array']['class'] = 'stregtui_sliderlayer rev_slider_wrapper fullwidthbanner-container';
  $style = array();
  ...
  foreach($slides as $slide){
    if($slide->status){
      ...
      $vars['content'] .= stregtui_sliderlayer_slide($slide_settings, $i);
      $i = $i + 1;
    }
  }
...
function stregtui_sliderlayer_slide($vars, $index){
  global $base_url;
  $module_path = drupal_get_path('module', 'stregtui_sliderlayer');
  $slide = $vars['slide'];
  $layers = $slide->layers;
  ...
  if(isset($slide->overlay_enable) && $slide->overlay_enable=='on'){
    $vars['attributes_array']['class'] = 'stregtui-overlay ';
  }
  ...
  foreach($layers as $layer){
    ...
    $vars['content'] .= stregtui_sliderlayer_layer($layer_settings);
  }
...
function stregtui_sliderlayer_layer($vars){
  global $base_url;
  $module_path = drupal_get_path('module', 'stregtui_sliderlayer');
  $layer = $vars['layer'];
  $scount = $vars['scount'];
  ...
  if($layer->incomingclasses){
    $tmp = stregtuiGetArrAnimations()[$layer->incomingclasses];
    if($tmp){
  ...
  if($layer->outgoingclasses){
    $tmp_out = stregtuiGetArrEndAnimations()[$layer->outgoingclasses];
