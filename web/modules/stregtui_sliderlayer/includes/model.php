<?php
function stregtui_sliderlayer_load($sid) {
  $result = \Drupal::database()->select('{stregtui_sliderlayers}', 'd')
      ->fields('d')
      ->condition('id', $sid, '=')
...
function stregtui_sliders_by_group($gid=0) {
  $result = \Drupal::database()->select('{stregtui_sliderlayers}', 'd')
          ->fields('d')
...
function stregtui_slider_load_frontend($sid=0) {
  ...
  $slides = \Drupal::database()->select('{stregtui_sliderlayers}', 'd')
            ->fields('d')
...
function getListSliderGroups(){
    $result = \Drupal::database()->select('{stregtui_sliderlayergroups}', 'd')
          ->fields('d')
          ->execute()
          ->fetchObject();
    return $result;
...
function stregtui_sliderlayer_export($gid){
  $result = new stdClass();
    ...
    $sliders = stregtui_sliders_by_group($gid);
