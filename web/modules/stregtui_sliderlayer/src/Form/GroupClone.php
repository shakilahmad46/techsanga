...
class GroupClone implements FormInterface {
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
      ...
      if (is_numeric($sid)) {
        $slide = \Drupal::database()->select('{stregtui_sliderlayergroups}', 'd')
                 ->fields('d')
                 ->condition('id', $sid, '=')
                 ->execute()->fetchAssoc();
        } else {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if (is_numeric($form['id']['#value']) && $form['id']['#value'] > 0) {
      ...
      $new_gid = $builder = \Drupal::database()->insert("stregtui_sliderlayergroups")
      ->fields(array(
      ...
      $slides = stregtui_sliders_by_group($old_id);
