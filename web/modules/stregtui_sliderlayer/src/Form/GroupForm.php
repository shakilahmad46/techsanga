...
class GroupForm implements FormInterface {
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
      ...
      if (is_numeric($sid)) {
        $slide = \Drupal::database()->select('{stregtui_sliderlayergroups}', 'd')->fields('d')->condition('id', $sid, '=')->execute()->fetchAssoc();
        } else {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if (is_numeric($form['id']['#value']) && $form['id']['#value'] > 0) {
      $sid = $builder = \Drupal::database()->update("stregtui_sliderlayergroups")
        ->fields(array(
      ...
      \Drupal::messenger()->addMessage("Slide '{$form['title']['#value']}' has been updated");
        ...
        $sid = $builder = \Drupal::database()->insert("stregtui_sliderlayergroups")
          ->fields(array(
              'title' => $form['title']['#value'],
              'params' => ''
          ))
