...
class ImportForm implements FormInterface {
   ...
   */
   public function buildForm(array $form, FormStateInterface $form_state) {
      ...
      if (is_numeric($gid)) {
        $group = \Drupal::database()->select('{stregtui_sliderlayergroups}', 'd')
           ->fields('d')
           ->condition('id', $gid, '=')
           ->execute()
           ->fetchAssoc();
      ...
      if($group['id']==0){
        \Drupal::messenger()->addMessage('Not found stregtui  slider layer !');
        return false;
      }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form['gid']['#value']) {
      ...
      $builder = \Drupal::database()->update("stregtui_sliderlayergroups")
      ->fields(array(
      ...
      if($slideshow->sliders){
        \Drupal::database()->delete('stregtui_sliderlayers')->condition('group_id', $gid)->execute();
        foreach ($slideshow->sliders as $key => $slider) {
          $i++;
          $builder = \Drupal::database()->insert("stregtui_sliderlayers")
            ->fields(array(
      ...
      $response = new \Symfony\Component\HttpFoundation\RedirectResponse(Url::fromRoute('stregtui_sl_group.admin', array('gid'=>$gid))->toString());
      $response->send();
    }
  }
}
