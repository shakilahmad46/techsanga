  ...
  * $Desc
  *
  * @author     Stregtui <stregtuithemes@gmail.com>
...
(function ($) {
  $.fn.extend({insertcaret:function(t){return this.each(function(){if(document.selection){this.focus();var e=document.selection.createRange();e.text=t,this.focus()}else if(this.selectionStart||"0"==this.selectionStart){var s=this.selectionStart,i=this.selectionEnd,n=this.scrollTop;this.value=this.value.substring(0,s)+t+this.value.substring(i,this.value.length),this.focus(),this.selectionStart=s+t.length,this.selectionEnd=s+t.length,this.scrollTop=n}else this.value+=t,this.focus()})}});
   ...
   $(document).ready(function () {
      ...
      if(isInIFrame==true){
        ...
        $('#stregtui-blockbuilder-setting').css('padding-top', '20px');
      }
      ...
      $('#stregtui-blockbuilder-setting').delegate('#save', 'click', function(){
        $(this).attr('disabled', 'true');
         stregtui_save_blockbuilder();
         return false;
      })
  ...
  function stregtui_save_blockbuilder(){
      var result = $('#stregtui-blockbuilder-setting input:not(.input-file-upload), #stregtui-blockbuilder-setting select, #stregtui-blockbuilder-setting textarea').serializeObject();
      result = $.extend({}, result);
      result =base64Encode(JSON.stringify(result));
        //console.log(result);
      var pid = $("input[name=stregtui_blockbuilder_id]").val();
      var data = {
         data: result,
         pid: pid
      };
      ...
      $.ajax({
         url: drupalSettings.stregtui_blockbuilder.saveConfigURL,
         type: 'POST',
         data: data,
         dataType: 'json',
         success: function (data) {
          ...
          $('#save').removeAttr('disabled');
          if(drupalSettings.stregtui_blockbuilder.destination==true){
            window.location = drupalSettings.stregtui_blockbuilder.url_redirect;
          }
   ...
   function init_sortable_find(wrap, find, items){
      wrap.find(find).sortable({ 
         start: function(e, ui){
           ui.placeholder.width(ui.item.find('.stregtui-blockbuilder-content').first().width() - 20);
  ...
  function gbb_sortable_elements(){
   ...
   function stregtui_block_builder(){
    ...
    var rowid = $('#gbb-row-id');
   	$('.bb-btn-row-add').click(function(){
        ...
        init_sortable_find(clone, _bb_sortable, _bb_item);
   		...
   		clone.hide().find('.stregtui-blockbuilder-content > input').each(function() {
    ...
    // form
      ...
      oresult = '';
      $('#gbb-form-setting').parent().find('.stregtui-overlay').first().addClass('active').parents('body').addClass('stregtui-overflow-hidden');
      ...
      if(drupalSettings.stregtui_blockbuilder.check_imce=='on'){
        load_imce();
      }
      return;
   	});
    ...
    $('#gbb-form-setting .gbb-form-setting-save').click(function(){
        ...
        iresult.find('>.stregtui-blockbuilder-content>.gbb-item-content>.item-bb-title').html(iresult.find('.display-admin').first().val());
        popup.fadeOut(50, function(){
          $(this).parent().find('.stregtui-overlay').first().removeClass('active').parents('body').removeClass('stregtui-overflow-hidden');
        })
    ...
    $('#gbb-form-setting .gbb-form-setting-cancel').click(function(){
      ...
      popup.fadeOut(50).parent().find('.stregtui-overlay').first().removeClass('active').parents('body').removeClass('stregtui-overflow-hidden');
      setTimeout(function(){
        popup.find('.gbb-el-meta').remove();
      }, 150);
      iresult = ''; oresult = '';
  ...
  var gvaImceInput = window.gvaImceInput = window.gvaImceInput || {
     urlSendto: function(File, win) {
      ...
      win.close();
      if (el) {
        var base_path = drupalSettings.stregtui_blockbuilder.base_path;
