<?php
use Drupal\stregtui_blockbuilder\includes\core\stregtui_sc;

function stregtui_single_field( $field, $meta ){
	$output = '';
	if( isset( $field['type'] ) ){ ?>
		<div class="single-field clearfix">
			...
			<?php 
				$bb_field = new stregtui_bb_fields();
...
function stregtui_admin_element( $item_std, $item = false, $column_id = false, $row_id = false) {
	...
	if(!$label){
		$label = ( $item && key_exists('fields', $item) && key_exists('title', $item['fields']) ) ? stregtui_render_textlangs($item['fields']['title']) : '';
	}	
	ob_start();
  	include STREGTUI_BLOCKBUILDER_PATH . '/templates/backend/admin-element.php';
  	$content = ob_get_clean();
	print $content;				
}

function stregtui_admin_row( $item_std, $row_std, $column_std, $row = false, $row_id = false ) {
	$name_row_id = $row ? 'name="gbb-row-id[]"' : '';
	ob_start();
  	include STREGTUI_BLOCKBUILDER_PATH . '/templates/backend/admin-row.php';
  	$content = ob_get_clean();
	print $content;		
}

function stregtui_admin_column( $item_std, $column_std, $column = false, $column_id = false,  $row_id = false ) {
	...
	ob_start();
  	include STREGTUI_BLOCKBUILDER_PATH . '/templates/backend/admin-column.php';
  	$content = ob_get_clean();
	print $content;		
}

function stregtui_blockbuilder_admin($pid) {
	$pbd_single = stregtui_blockbuilder_load($pid);
	if(!$pbd_single){
	...
	ob_start();
  	include STREGTUI_BLOCKBUILDER_PATH . '/templates/backend/admin-builder.php';
  	$content = ob_get_clean();
	print $content;
}
