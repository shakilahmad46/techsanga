<?php
namespace Drupal\stregtui_blockbuilder\includes\core;
class stregtui_sc{
  ...
  public function gsc_load_shortcodes($backend=true){
    if( empty($this->gv_shortcodes) ){
      $shortcodes = [];
      $theme_name = stregtui_blockbuilder_theme_name();

      //Load element from themes
      if(file_exists(drupal_get_path('theme', $theme_name) . '/gva_elements/elements.php')){
        
        $shortcodes = [];
        if(function_exists('stregtui_blockbuilder_get_elements')) $shortcodes = stregtui_blockbuilder_get_elements();
        foreach( $shortcodes as $sc ){
          ...
          if($sc_path){
            $class = $sc;
            $_class = '\\Drupal\stregtui_blockbuilder\shortcodes\\'.$class;
      ...
      //Load elements from stregtui_elements modules (financial, charityplus ,sanbro, carina)
      }else{

        if(function_exists('stregtui_elements_list_shortcodes')){
          $shortcodes = stregtui_elements_list_shortcodes();
        }
        foreach( $shortcodes as $sc ){
          $sc_path = '';
          if(file_exists(drupal_get_path('module', 'stregtui_elements') . '/elements/' . $sc . '.php')){
            $sc_path = drupal_get_path('module', 'stregtui_elements') . '/elements/' . $sc . '.php';
          }
          if($sc_path){
            $class = $sc;
            $_class = '\\Drupal\stregtui_blockbuilder\shortcodes\\'.$class;
