...
/**
 * @file
 * Contains \Drupal\stregtui_blockbuilder\Controller\GBBAdminController.
 */

namespace Drupal\stregtui_blockbuilder\Controller;
...
class GBBAdminController extends ControllerBase {
  ...
  public function stregtui_blockbuilder_list(){

    $page['#attached']['library'][] = 'stregtui_blockbuilder/stregtui_blockbuilder.assets.admin';
    ...
    foreach ($results as $row) {
      ...
      $tmp[] = t('<a href="@link">Edit</a> | <a href="@link_2">Config</a> |  <a href="@link_3">Delete</a> | <a href="@link_4">Duplicate</a>', array(
            '@link' => Url::fromRoute('stregtui_blockbuilder.admin.add', array('bid' => $row->id))->toString(),
            '@link_2' => Url::fromRoute('stregtui_blockbuilder.admin.edit', array('bid' => $row->id))->toString(),
            '@link_3' => Url::fromRoute('stregtui_blockbuilder.admin.delete', array('bid' => $row->id))->toString(),
            '@link_4' => Url::fromRoute('stregtui_blockbuilder.admin.clone', array('bid' => $row->id))->toString()
        ));
      $rows[] = $tmp;
    }
    $page['gbb-admin-list'] = array(
      ...
      '#header' => $header,
      '#rows' => $rows,
      '#empty' => t('No Block builder block available. <a href="@link">Add Block Builder</a>.', array('@link' => Url::fromRoute('stregtui_blockbuilder.admin.add', array('bid'=>0))->toString() )),
    );
    return $page;
  }

  public function stregtui_blockbuilder_save_element($data) {
    $gbb_els = array();
  ...
  public function stregtui_blockbuilder_edit($bid) {
    require_once STREGTUI_BLOCKBUILDER_PATH . '/includes/utilities.php';
    require_once STREGTUI_BLOCKBUILDER_PATH .'/includes/backend.php';
    
    $page['#attached']['library'][] = 'stregtui_blockbuilder/stregtui_blockbuilder.assets.admin';
    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['check_imce'] = 'off';
    if(stregtui_blockbuilder_imce()) {
      $page['#attached']['library'][] = 'imce/drupal.imce.input';
      $page['#attached']['drupalSettings']['stregtui_blockbuilder']['check_imce'] = 'on';
    }

    $page['#attributes']['classes_array'][] = 'form-blockbuilder';

    $abs_url_config = Url::fromRoute('stregtui_blockbuilder.admin.save', array(), array('absolute' => FALSE))->toString(); 
    
    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['saveConfigURL'] = $abs_url_config;

    $abs_url_config = Url::fromRoute('stregtui_blockbuilder.admin.get_images_upload', array(), array('absolute' => FALSE))->toString();

    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['get_images_upload_url'] = $abs_url_config;

    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['base_path'] = base_path();
    ...
    if(isset($_GET['destination']) && $_GET['destination']){
      ...
      $url_redirect = Url::fromRoute('stregtui_blockbuilder.admin.edit', array('bid' => $bid))->toString();
    }

    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['url_redirect'] = $url_redirect;
    
    $page['#attached']['drupalSettings']['stregtui_blockbuilder']['destination'] = $destination;

    ob_start();
    include drupal_get_path('module', 'stregtui_blockbuilder') . '/templates/backend/form.php';
  ...
  public function stregtui_blockbuilder_save(){
    ...
    if($data){
      ...
      $params = $this->stregtui_blockbuilder_save_element($data);
    } 
  ...
  public function stregtui_blockbuilder_export($bid){
    $pbd_single = stregtui_blockbuilder_load($bid);
  ...
  public function stregtui_upload_file(){
    ...
    $_id = stregtui_blockbuilder_makeid(6);
