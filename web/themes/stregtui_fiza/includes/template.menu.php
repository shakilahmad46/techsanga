...
function stregtui_fiza_preprocess_menu__main(&$variables) {
  $variables['attributes']['class'][] = 'clearfix';
  foreach ($variables['items'] as &$item) {
    $menu_link_attributes = _stregtui_fiza_attributes_get_attributes($item['original_link']);
    if (count($menu_link_attributes)) {
      ...
      if(isset($attributes['gva_layout']) && $attributes['gva_layout']=='menu-block'){
        $item['gva_block_content'] = stregtui_fiza_render_block($attributes['gva_block']);
      }
    }
  }
}

function _stregtui_fiza_attributes_get_attributes(MenuLinkInterface $menu_link_content_plugin) {
  $attributes = [];
  try {
