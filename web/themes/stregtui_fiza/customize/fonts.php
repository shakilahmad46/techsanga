<?php
function stregtui_fiza_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function stregtui_fiza_render_option_font(){
    $fonts = stregtui_fiza_fonts();
    $output = '';
...
function stregtui_fiza_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function stregtui_fiza_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(stregtui_fiza_fonts())) > 17){
...
function stregtui_fiza_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= stregtui_fiza_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= stregtui_fiza_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function stregtui_fiza_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'stregtui_fiza');
